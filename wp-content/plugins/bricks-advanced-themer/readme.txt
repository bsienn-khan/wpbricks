...
= 3.3.8 ( OCT 28, 2025)
* FIX: The variable picker was triggered when clicking on the Bricks CSS icon event when the event was set to right-click 
* FIX: Imported classes could be detached from the element on a recent update
* FIX: Wrong background-color on element shortcut icons when pseudo-element shortcuts are deactivated
* FIX: Force Theme to dark could fail
* FIX: Mobile-first page.css could be missing in Advanced CSS 
* FIX: Keyboard shortcuts for Grid Guides, Xmode, Contrast Checker and Darkmode stopped working correctly
* FIX: ColorPicker within the Color Manager could wrongly resolve valid hex colors 
* FIX: PHP Warnings when the Theme Settings haven't been saved yet
* FIX: Quick Remote Templates wasn't working anymore since Bricks 2.1
* FIX: Class Manager: Renaming an active global class could fail being persistant on class change
